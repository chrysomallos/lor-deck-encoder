...
const ROOT_PATH_TEMPLATE = 'core/{language}/data/globals-{language}.json';
const SET_PATH_TEMPLATE = '{name}/{language}/data/{name}-{language}.json';
...
export default class DataDragon {
  ...
  async initialize(language) {
    ...
    if (!cards || !core) {
      core = await request(new URL(ROOT_PATH_TEMPLATE.replace(/\{language\}/g, language.toLocaleLowerCase()), this.baseUrl));
